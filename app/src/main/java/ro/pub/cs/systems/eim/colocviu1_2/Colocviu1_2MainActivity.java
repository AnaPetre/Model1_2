...
public class Colocviu1_2MainActivity extends AppCompatActivity {
        ...
        public void onClick(View v) {
            ...
            if (v.getId() == R.id.compute_button)
            {
                ...
                else
                {
                   ...
                   // cheia este array, iar valoarea este sumArray
                    intent.putExtra("array", sumArray); // trimis ca parametru al intentiei => vectorul efectiv
                    // cheia este NUM_VALUES, iar valoarea este idx
                    intent.putExtra(Constants.NUM_VALUES, idx); // trimis ca parametru al intentiei => nr elemente din vector
                    startActivityForResult(intent, Constants.REQUEST_CODE);
                }

                // valoarea noua va fi inlocuita cu noua suma mereu
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (requestCode == Constants.REQUEST_CODE)
        {
            ...
            // daca aveam vreo intentie de la activitatea secundara
            if (intent != null)
            {
                Log.d(Constants.MAIN, "Message found");
                // totalSum il luam din param Extra primit la intentie
                totalSum = intent.getIntExtra(Constants.SECOND_RETURN_KEY, -1);
                // punem mesajul in Toast
        ...
        if (totalSum >= 10)
        {
            // mesaj cand pornim serviciul
            Toast.makeText(getApplicationContext(), "Service started", Toast.LENGTH_LONG).show();

            // facem o noua intentie catre Colocviu1_2Service.class
            // primul arg va fi mereu getApplicationContext()
            // al doilea arg va fi clasa catre care facem intentia
            Intent intentService = new Intent(getApplicationContext(), Colocviu1_2Service.class);

            // in intentie vom pune valoarea sumei daca aceasta este >= 10
            intentService.putExtra(Constants.CALCULATED_SUM, totalSum);

            // pornim serviciul cu ajutorul intentiei
    ...
    protected void onSaveInstanceState(Bundle savedInstanceState) {
        ...
        // ce punem in starea de save: suma calculata
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        // daca am avut vreo suma salvata anterior
        if (savedInstanceState.containsKey(Constants.CALCULATED_SUM)){
            // vom afisa valoarea sumei salvate
